    ...
    queryFn: async () => {
      try {
        const response = await axiosInstance.get(
          ...
          agent_metadata_validation_status=${payload?.agent_metadata_validation_status}&agent_table_data_validation_status=${payload?.agent_table_data_validation_status}`
        );
        return response?.data;
      } catch (error) {
        return error?.response?.data;
