    ...
    <div className={`!relative flex  my-4 w-full !justify-between  items-start h-full bg-[#FAFAFA]`}>
      {timeline?.map(({ order, note, reached, timestamp, state }, index) => (
        <div key={order} className="relative flex w-full items-center  ">
          ...
          {index > 0 && (
            <div className={`${reached && order!==length-1 &&"!bg-primary"} absolute -left-[86.1%]  top-2 h-0.5  z-10 w-full bg-[#C6C6C6]`}></div>
          )}
          ...
          <CustomTooltip content={note}>
            <div className="!text-[#6F6F6F] flex flex-col !z-50 items-center font-lato text-xs font-medium">
              <div
                className={`${
                  reached ? "bg-primary !border-primary" : "border-[#C6C6C6] bg-white"
