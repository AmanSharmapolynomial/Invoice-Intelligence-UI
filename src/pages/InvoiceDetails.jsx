...
const markDownComponents = {
  strong: ({ children }) => (
    <strong className="font-semibold font-poppins text-xs capitalize text-left text-[#111827]">
      {children}
    </strong>
  ),
  h2: ({ children }) => (
    <h2 className="font-poppins font-medium text-sm">{children}</h2>
  ),
  ul: ({ children }) => (
    <ul className="list-disc font-poppins text-xs  text-left space-y-1 ">
      {children}
    </ul>
  ),
  ol: ({ children }) => (
    <ol className="list-decimal font-poppins text-xs ml-5 mb-1.5 font-normal text-left">
      {children}
    </ol>
  ),
  p: ({ children }) => (
    <p className=" last:mb-0 font-poppins font-normal !text-xs text-left  !my-2">
      {children}
    </p>
  ),
  table: ({ children }) => (
    <div className="overflow-x-auto my-2">
      <table className="min-w-full font-poppins border border-chineseWhite rounded-md !text-xs">
        {children}
      </table>
    </div>
  ),
  thead: ({ children }) => (
    <thead className="bg-gray-100 font-poppins text-xs border-b border-chineseWhite text-left">
      {children}
    </thead>
  ),
  tbody: ({ children }) => (
    <tbody className="divide-y font-poppins text-xs divide-chineseWhite">
      {children}
    </tbody>
  ),
  tr: ({ children }) => (
    <tr className="hover:bg-gray-50 font-poppins text-xs transition-colors">
      {children}
    </tr>
  ),
  th: ({ children }) => (
    <th className=" !w-[7.5rem] break-words  font-semibold text-gray-700 border-r border-chineseWhite whitespace-nowrap">
      <div className="w-[7.5rem] font-poppins text-xs px-3 py-2 break-words whitespace-normal h-full flex  items-start">
        {children}{" "}
      </div>
    </th>
  ),
  td: ({ children }) => (
    <td className="text-gray-600 border-r text-left !w-[7.5rem] break-words border-chineseWhite whitespace-nowrap">
      <div className="w-[7.5rem] font-poppins text-xs px-3 py-2 break-words whitespace-normal h-full flex  items-start">
        {children}{" "}
      </div>
    </td>
